...
class ProductsPage_book extends StatefulWidget {
  ProductsPage_book({Key? key,required this.vid,required this.cat}) : super(key: key);
  var vid;

  var cat;
  @override
  State<ProductsPage_book> createState() => _ProductsPage_bookState();
}

class _ProductsPage_bookState extends State<ProductsPage_book> {
  var flag;
  var pid;
  ...
  Future<dynamic> editData() async {
    ...
    var response =
    await post(Uri.parse('${Con.url}editOther.php'), body: data);
    print(response.body);
    print(response.statusCode);
    if (response.statusCode == 200) {
      if(jsonDecode(response.body)['result'] == 'success'){
                          ...
                          children: [
                            Padding(
                              ...
                              child: Container(
                                  ...
                                  child: Image.network(
                                    '${Con.url}bookUploads/${snapshot.data[index]['image']}',
                                    fit: BoxFit.fill,
                                  )),
                            ),
                            Padding(
                            ...
                            Padding(
                              padding: const EdgeInsets.all(8.0),
                              child: Text(
                                '${snapshot.data[index]['stock']} Left',
                               // style: heading1,
                              ),
                            ), Padding(
                              padding: const EdgeInsets.all(8.0),
                              ...
                              padding: const EdgeInsets.all(8.0),
                              child: Text(
                                'Publications:${snapshot.data[index]['publishyr']}',
                               // style: heading1,
                              ),
                            ),
                            Padding(
