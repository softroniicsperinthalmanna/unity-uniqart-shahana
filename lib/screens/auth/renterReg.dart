...
import 'login.dart';
class RenterReg extends StatefulWidget {
  RenterReg({Key? key}) : super(key: key);
  // var userType;
  @override
  State<RenterReg> createState() => _RenterRegState();
}

class _RenterRegState extends State<RenterReg> {
  ...
  Future<void> signUp() async {
    ...
    var response= await post(Uri.parse('${Con.url}renterReg.php'),body: data);
    print(response.body);
    print(response.statusCode);
    if(response.statusCode==200){
      if(jsonDecode(response.body)['result']=='success'){
        ScaffoldMessenger.of(context).showSnackBar(snackMsg('Registered....'));
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>const RenterLogin()));
        ...
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=> RenterReg()));
    ...
    return Scaffold(
      ...
      // ),
        body:Padding(
          ...
          child: ListView(
            children: [
              Center(child: Text('Create Renter Account',style:authHead,)),
              SizedBox(height: 20,),
