...
import 'login.dart';
class NeedyReg extends StatefulWidget {
  NeedyReg({Key? key}) : super(key: key);
  // var userType;
  @override
  State<NeedyReg> createState() => _NeedyRegState();
}

class _NeedyRegState extends State<NeedyReg> {
  ...
  Future<void> signUp() async {
    ...
    var response= await post(Uri.parse('${Con.url}needyReg.php'),body: data);
    print(response.body);
    print(response.statusCode);
    if(response.statusCode==200){
      if(jsonDecode(response.body)['result']=='success'){
        ScaffoldMessenger.of(context).showSnackBar(snackMsg('Registered....'));
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>const NeedyLogin()));
        ...
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=> NeedyReg()));
    ...
    return Scaffold(
      ...
      // ),
        body:Padding(
          ...
          child: ListView(
            children: [
              Center(child: Text('Create Needy Account',style:authHead,)),
              SizedBox(height: 20,),
